...
def prefix_causality_audit(
    ...
    """
    Rebuild sampled signals using only candles available through the original
    entry candle.

    Matching primarily uses entry_index because timestamp-only matching can
    fail after dataframe normalization or timezone conversion.
    """
    ...
    candles = (
        candles
        .dropna(subset=[candle_timestamp_column])
        .sort_values(candle_timestamp_column)
        .reset_index(drop=True)
    )

    accepted_working = accepted.copy()

    if "entry_index" not in accepted_working.columns:
        raise KeyError(
            "Accepted trade report does not contain entry_index."
        )

    accepted_working["entry_index"] = pd.to_numeric(
        accepted_working["entry_index"],
        errors="coerce",
    )

    accepted_working = accepted_working.dropna(
        subset=["entry_index"]
    )

    sample_count = min(checks, len(accepted_working))

    sample_positions = np.linspace(
        0,
        len(accepted_working) - 1,
        sample_count,
        dtype=int,
    )

    sampled = accepted_working.iloc[sample_positions]
    research_config = ResearchConfig(instrument=instrument)

    rows: list[dict[str, Any]] = []

    for _, accepted_row in sampled.iterrows():
        original_entry_index = int(accepted_row["entry_index"])
        original_timestamp = pd.to_datetime(
            accepted_row.get("timestamp"),
            utc=True,
            errors="coerce",
        )
        original_direction = str(
            accepted_row.get("direction", "")
        ).lower()
        ...
        # Include candles only through the original entry candle.
        prefix_end = original_entry_index + 1
        prefix = candles.iloc[:prefix_end].copy()

        prefix_last_timestamp = (
            prefix[candle_timestamp_column].iloc[-1]
            if not prefix.empty
            else pd.NaT
        )
        ...
        try:
            ...
            if not prefix_setups.empty:
                if "entry_index" in prefix_setups.columns:
                    numeric_entry_index = pd.to_numeric(
                        prefix_setups["entry_index"],
                        errors="coerce",
                    )

                    matching = prefix_setups[
                        numeric_entry_index == original_entry_index
                    ].copy()

                    if not matching.empty:
                        matching_method = "entry_index"
                ...
                if (
                    not matching.empty
                    and "direction" in matching.columns
                    and original_direction
                ):
                    ...
                    if not direction_matching.empty:
                        matching = direction_matching
        ...
        rows.append(
            {
                ...
                "prefix_last_timestamp": prefix_last_timestamp,
