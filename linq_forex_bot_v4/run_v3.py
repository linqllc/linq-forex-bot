...
def main():
    p = argparse.ArgumentParser(description="LINQ Forex Research Bot v4: ML-ready features, target probabilities, condition analysis")
    ...
    for pair_number, instrument in enumerate(instruments, start=1):
        print(f"\n{'='*72}\nLINQ Forex Research Bot v4 | {instrument} | pair {pair_number}/{len(instruments)}\n{'='*72}")
        ...
        research_timer = StageTimer(f"Building research dataset for {instrument}")
        features = build_feature_dataset(setups, candles, ratios)
        probabilities = target_probability_table(features, ratios)
        selected_ratio = float(selection["best_overall_ratio"])
        conditions = condition_breakdown(features, selected_ratio)
        feature_stats = feature_summary(features)
        research_timer.done(f"{len(features)} ML-ready rows")
        ...
        features.to_csv(out/f"{instrument}_{source}_ml_dataset.csv", index=False)
        probabilities.to_csv(out/f"{instrument}_{source}_target_probabilities.csv", index=False)
        conditions.to_csv(out/f"{instrument}_{source}_condition_breakdown.csv", index=False)
        feature_stats.to_csv(out/f"{instrument}_{source}_feature_summary.csv", index=False)
    ...
    print(f"\n✓ Research run complete. Reports: {out}")
