...
def _zone_is_invalidated(
    row: pd.Series,
    zone_bottom: float,
    zone_top: float,
    zone_type: ZoneType,
) -> bool:
    """Return whether this candle closes through the zone."""
    if zone_type == "demand":
        return float(row["mid_close"]) < zone_bottom

    return float(row["mid_close"]) > zone_top


def _find_zone_invalidation_index(
    frame: pd.DataFrame,
    start_index: int,
    zone_top: float,
    zone_bottom: float,
    ...
    """Return the first candle index that invalidates the zone."""
    for index in range(start_index, len(frame)):
        row = frame.iloc[index]

        if _zone_is_invalidated(
            row,
...
def _count_zone_touches(
    ...
    """Count distinct zone entries before invalidation."""
    touches = 0
...
def detect_automatic_zones(
    ...
    while index < len(frame) - minimum_impulse_candles:
        ...
        invalidation_index = _find_zone_invalidation_index(
            frame,
            impulse_end + 1,
            zone_top,
            zone_bottom,
...
def generate_automatic_setups(
    ...
    for _, zone in zones.iterrows():
        ...
        invalidation_index = zone.get("invalidation_index")

        if pd.notna(invalidation_index):
            end = min(
                end,
                int(invalidation_index) + 1,
            )
        ...
        for index in range(start, end):
            ...
            if direction == "long":
                zone_invalid = _zone_is_invalidated(
                    row,
    ...
    columns = [
        field.name
        for field in fields(AutomaticSetup)
    ]
