    ...
    def get_candles(
        ...
        while cursor < end.astimezone(timezone.utc):
            ...
            if progress_callback:
                completed = min(total_seconds, max(0, int((cursor - start).total_seconds())))
                progress_callback(completed, total_seconds, f"{len(rows):,} candles")
            time.sleep(0.05)
        ...
        if progress_callback:
            progress_callback(total_seconds, total_seconds, f"{len(rows):,} candles")
