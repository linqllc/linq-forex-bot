...
def run_strategy(df: pd.DataFrame, instrument: str, config: dict, reward_to_risk: float | None = None) -> pd.DataFrame:
    ...
    reward_risk = float(reward_to_risk if reward_to_risk is not None else risk["reward_to_risk"])
